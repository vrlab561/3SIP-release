...
def compute_euler_angles_from_rotation_matrices(rotation_matrices):
    ...
    #judge = np.array()
    #singular = torch.zeros_like(singular_)
    #zeros = torch.zeros((rotation_matrices.shape[0]))
    #ones = torch.ones((rotation_matrices.shape[0]))
    #singular = torch.where(singular < 1e-6, ones, zeros)
...
def compute_quaternions_from_rotation_matrices(matrices):
    ...
    tmp = 1.0 + matrices[:,0,0] + matrices[:,1,1] + matrices[:,2,2]
    w = torch.sqrt(tmp+1e-6) / 2.0
    #w =
    ...
    #w = torch.max(w, torch.autograd.Variable(torch.zeros(batch)) + 1e-8)
    w4 = 4.0 * w;
...
# input jointnum*3*3
# output jointnum*3
def compute_AxisAngle_from_rotation_matrices(SO3Array):
    res = np.zeros((SO3Array.shape[0],3))
    for j in range(SO3Array.shape[0]):
        matrix = SO3Array[j].cpu().numpy()
...
def joints65to22(input):    #joints_num*...
    out_22_joints = input[0:5, ]
    out_22_joints = torch.cat([out_22_joints, input[6:10, ]], dim=0)
    out_22_joints = torch.cat([out_22_joints, input[11:16, ]], dim=0)
    out_22_joints = torch.cat([out_22_joints, input[17:21, ]], dim=0)
    out_22_joints = torch.cat([out_22_joints, input[41:45, ]], dim=0)
    return out_22_joints

def joints65to22_batch(input):    #joints_num*...
    out_22_joints = input[:,0:5,]
    out_22_joints = torch.cat([out_22_joints, input[:,6:10, ]], dim=1)
    out_22_joints = torch.cat([out_22_joints, input[:,11:16, ]], dim=1)
    out_22_joints = torch.cat([out_22_joints, input[:,17:21, ]], dim=1)
    out_22_joints = torch.cat([out_22_joints, input[:,41:45, ]], dim=1)
    return out_22_joints
...
def sixd2aa(pose_6d, batch:bool=False):
    """
    :param pose_6d: Nx6
    :return: pose_aa: Nx3
    """
...
if __name__=="__main__":
    aa = np.array([0.5418,0.0936,0.0742])
    aa = torch.from_numpy(aa).view(1,-1)
    so3 = compute_rotation_matrix_from_axisAngle(aa)
    backaa = compute_quaternions_from_rotation_matrices(so3)
    exit()