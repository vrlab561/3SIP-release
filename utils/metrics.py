...
def mpjru(
    ...
    diff[diff > np.pi] = diff[diff > np.pi] - 2 * np.pi
    diff[diff < -np.pi] = diff[diff < -np.pi] + 2 * np.pi
    rot_error = torch.mean(torch.absolute(diff).reshape(-1,21,3)[...,[i-1 for i in upper_index],:].reshape(diff.shape[0],-1))
    return rot_error
...
def mpjrl(
    ...
    diff[diff > np.pi] = diff[diff > np.pi] - 2 * np.pi
    diff[diff < -np.pi] = diff[diff < -np.pi] + 2 * np.pi
    rot_error = torch.mean(torch.absolute(diff).reshape(-1,21,3)[...,[i-1 for i in lower_index if i > 0],:].\
                           reshape(predicted_position.shape[0],-1))
    return rot_error
